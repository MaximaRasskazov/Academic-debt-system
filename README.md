...
При `SEED_DEV_ACCOUNTS=true` (по умолчанию в dev) сидятся 4 аккаунта — по одному на каждую роль. Пароль у всех — `password`.
...
> 👨‍🎓 `teacher` и `student` нужны, чтобы проверять их интерфейсы **без эмулятора**: преподаватель ведёт пару демо-дисциплин, у студента — пара открытых долгов по ним. В норме реальные преподаватели и студенты приходят из эмулятора деканата через `sync.Service`.
>
